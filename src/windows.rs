...
// Got this from https://users.rust-lang.org/t/how-do-i-determine-if-i-have-admin-rights-on-windows/35710/8
use std::io::Error;
use std::mem;
use std::ptr;
...
pub fn get_current_username() -> String {
	let mut name = mem::MaybeUninit::<[u16; UNLEN as usize]>::uninit();
	let mut size: [u16; 1] = [UNLEN as u16];

	let name = unsafe {
		GetUserNameW(name.as_mut_ptr() as *mut _, size.as_mut_ptr() as *mut u32);
		name.assume_init()
	};

	String::from_utf16_lossy(if size[0] == 0 { &[] } else { &name[..(size[0] - 1) as usize] })
}
